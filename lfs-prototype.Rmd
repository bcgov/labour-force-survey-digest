...
ggplotly_lfs(
  unemploy_rt_provinces_report_year %>% 
 ggplot(aes(
    x = date,
    y = value,
    group = geo
  ...
  geom_point(aes(colour = fill_col,
    ...
    alpha = 0.6) +
  geom_line(aes(colour = fill_col), alpha = 0.8) +
...
industry_order <- factor(industry_change$industry, levels = industry_change$industry[order(industry_change$value, decreasing = TRUE)])
...
p1 <- industry_change %>% 
  ...
  ggplot() +
  geom_col(aes(metric_value, industry))
```
