...
🖱 Click or tap on graphs for interactive options:
...
Row 2 {data-height=200}
----------------------------------------------------------------

```{r}
report_prov_date <- format(max(lfc_province_tidy$date), "%b-%Y")
last_month <- format(max(lfc_province_tidy$date)%m-% months(1), "%b-%Y")
last_year_same_month <- format(max(lfc_province_tidy$date) %m-% years(1), "%b-%Y")
```

### Women Change in Recent B.C. Unemployment Rate

```{r}

##year over year change unemployment rate female
year_change_unemployment_bc_female <- lfc_province_tidy %>%
  filter(vector %in% c("v2064831", 'v2064830'),
         date %in% c(max(date) - months(12), max(date))
  ) %>%
  mutate(year_change = value - lag(value)) %>%
  filter(date == max(date))
...
year_change_unemployment_bc_female %>% 
  ungroup() %>% 
  select(labour_force_characteristics, value, month_change, year_change) %>% 
  ...
  mutate(name = case_when(
    name == "value" ~ report_prov_date,
    name == "month_change" ~ last_month,
    name == "year_change"  ~ last_year_same_month,
    TRUE ~ "there is a problem"
  )) %>% 
  gt() %>% 
  tab_style(
    style = cell_fill(color = main_colour),
    locations = cells_body(
      rows = 2:3)
  ) %>%
  fmt_percent("Unemployment rate", decimals = 1) %>% 
  fmt_number("Unemployment", decimals = 0) %>% 
  tab_style(
    style = cell_text(
      size = px(12),
      color = "#999",
...
### Month-Over-Month Change in Number of Jobs in B.C. for Youth (15 to 24 Years) by Gender in `r report_year` 
...
### Private Sector Year-Over-Year Change in Number of Jobs in B.C. {.value-box}
...
valueBox(
  ...
  caption = "Year-Over-Year Change in Number of Private Sector Jobs in B.C."
)
```


### Public Sector Year-Over-Year Change in Number of Jobs in B.C. {.value-box}
...
valueBox(
  value = paste0("Public Sector: ",
    ...
    signs(format = comma,
          add_plusses = TRUE)),
  caption = "Year-Over-Year Change in Number of Public Sector Jobs in B.C."
)
```


### Self-employed Year-Over-Year Change in Number of Jobs in B.C. {.value-box}
...
valueBox(
  value = paste0("Self-employed: ",
    ...
    signs(format = comma,
          add_plusses = TRUE)),
  caption = "Year-Over-Year Change in Number of Self-employed Jobs in B.C."
)
```
...
### Goods-Producing Sector Month-Over-Month Change in Number of Jobs in B.C. {.value-box}


```{r}
valueBox(
  value = paste0("Goods-producing: ",
                 employment_by_industry_tidy %>% 
  filter(vector == "v2057794",
         date == max(date)) %>% 
    pull(month_change) %>%
    signs(format = comma,
          add_plusses = TRUE)),
  caption = paste("Month-Over-Month Change in Number of Goods-producing Jobs in B.C. for", report_month, report_year)
)
```


### Services-Producing Sector Month-Over-Month Change in Number of Jobs in B.C. {.value-box}

```{r}
valueBox(
  value = paste0("Services-producing: ",
                 employment_by_industry_tidy %>% 
  filter(vector == "v2057800",
         date == max(date)) %>% 
    pull(month_change) %>%
    signs(format = comma,
          add_plusses = TRUE)),
  caption = paste("Month-Over-Month Change in Number of Services-producing Jobs in B.C. for", report_month, report_year)
)
```
...
p2 <- ggplot(industry_change, 
  ...
  geom_text(aes(label = signs(
     ...
     nudge_x = ifelse(industry_change$month_change > 0, 4000, -4000),
    colour = "grey20",
    size = 3) +
...
p3 <- ggplot(industry_change, 
  ...
  geom_text(aes(label = signs(
     ...
     nudge_x = ifelse(industry_change$month_change_percent > 0, .027, -.027),
    colour = "grey20",
    size = 3) +
  scale_x_continuous(expand = expansion(mult = c(.2, .2))) +
  labs(x = NULL,
...
p4 <- ggplot(industry_change, 
  ...
  geom_text(aes(label = signs(
     ...
     nudge_x = ifelse(industry_change$year_change_percent > 0, .1, -.1),
    colour = "grey20",
    size = 3) +
   scale_x_continuous(expand = expansion(mult = c(.2, .2))) +
  labs(x = NULL,
