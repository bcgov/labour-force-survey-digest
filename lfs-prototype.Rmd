---
title: "Labour Force Survey: British Columbia Summary"
output: 
...
### Year Change in Unemployment Rate {.value-box}

```{r}
#year over year change unemployment rate
year_change_unemployment_rate <- lfc_province_tidy %>%
  filter(vector == "v2064705",
  date %in% c(max(date) - months(12), max(date))) %>% 
...
valueBox(
  value = year_change_unemployment_rate %>%
    pull(year_change) %>%
    signs(
      format = percent,
      add_plusses = TRUE,
...
### Recent Month Change in Canada Employment {.value-box}

```{r}
#recent monthly change in CAN employment
can_recent_month_change_employment <- lfc_province_tidy %>%
  filter(vector == "v2062811",
         date == max(date))

valueBox(
  value = can_recent_month_change_employment %>%
    pull(month_change) %>%
    signs(format = comma,
          add_plusses = TRUE),
  icon = "fa-users-cog",
  caption = paste("Change in Canada's Employment in", report_month)
)
```
...
can_recent_month_unemployment_rate <- lfc_province_tidy %>%
  filter(vector == "v2062815",
         date == max(date))

valueBox(
  value = can_recent_month_unemployment_rate %>%
    pull(value) %>%
    signs(format = percent, accuracy = 0.1),
  icon = "fa-users-cog",
  caption = paste("Canada's Unemployment Rate in", report_month)
)
```
...
### Change in Employment by Province in `r report_month`
...
ggplotly(p9, tooltip = c("text"))
```


### Monthly Change in Unemployment Rates by Province in `r report_year`


```{r, fig.width=7, fig.height=5}
p10 <- lfc_province_tidy %>%
  filter(
    labour_force_characteristics == "Unemployment rate",
    sex == "Both sexes",
    age_group == "15 years and over",
...
### CAN Year Change in Employment {.value-box}

```{r}
can_year_change_employment <- lfc_province_tidy %>%
  filter(vector == "v2062811",
         date %in% c(max(date) - months(12), max(date))) %>% 
  mutate(
    year_change = value - lag(value),
...
#year over year change employment
valueBox(value = can_year_change_employment %>% 
  pull(year_change) %>%
  signs(format = comma,
        add_plusses = TRUE),
         icon = "fa-users-cog",
         caption = paste("Year-Over-Year Change in Canada's Employment"))
```


### CAN Year Change in Unemployment Rate {.value-box}

```{r}
#CAN year over year change unemployment rate
can_year_change_unemployment_rate <- lfc_province_tidy %>%
  filter(vector == "v2062815",
  date %in% c(max(date) - months(12), max(date))) %>% 
  mutate(year_change = value - lag(value)) %>% 
  filter(date == max(date))
...
valueBox(
  value = can_year_change_unemployment_rate %>%
    pull(year_change) %>%
    signs(
      format = percent,
      add_plusses = TRUE,
  ...
  icon = "fa-users-cog",
  caption = paste("Year-Over-Year Change in Canada's Unemployment Rate")
)
```
