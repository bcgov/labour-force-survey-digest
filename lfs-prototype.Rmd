...
🖱 Click️ or tap on graph & maps for interactive options:
...
ggplotly_lfs(unemploy_rt_bc)
```

### Unemployment Rates for `r report_month` by B.C. Economic Region 

```{r, fig.width=7, fig.height=5}
unemploy_rt_bc_regions <- lfc_region_tidy %>%
  filter(
...
unemploy_rt_bc_regions

# ggplotly_lfs(unemploy_rt_bc_regions) 
```
...
```{r, fig.width=7, fig.height=5}
mom_change_employment_bc <- lfc_province_tidy %>%
  filter(
    vector == "v2064701",
    date >= paste0(report_year, "-01-01")
  ) %>%
...
ggplotly_lfs(mom_change_employment_bc)
```

### Month-Over-Month Change in Number of Jobs in B.C. by Job Type in `r report_year` 

```{r, fig.width=7, fig.height=5}
mom_change_employment_bc_jobtype<- lfc_province_tidy %>%
  filter(
    date >= paste0(report_year, "-01-01"),
    geo == "British Columbia",
...
ggplotly_lfs(mom_change_employment_bc_jobtype)
```
...
```{r, fig.width=7, fig.height=5}
mom_change_employment_bc_sector <- employment_by_class_tidy %>%
  filter(
...
ggplotly_lfs(mom_change_employment_bc_sector)
```
...
### Unemployment Rate for `r report_month` by Province
...
unemploy_rt_provinces_map <- can_provinces %>% 
...
# unemploy_rt_provinces_map

ggplotly_lfs(unemploy_rt_provinces_map)
```

### Monthly Unemployment Rates by Province in `r report_year`

```{r, fig.width=7, fig.height=5}
unemploy_rt_provinces_timeseries <- lfc_province_tidy %>%
  filter(
    labour_force_characteristics == "Unemployment rate",
    sex == "Both sexes",
    age_group == "15 years and over",
...
ggplotly_lfs(unemploy_rt_provinces_timeseries)
```
...
### Month-Over-Month Change in Number of Jobs by Province in `r report_year`
...
change_employment_provinces <- lfc_province_tidy %>%
  filter(
    labour_force_characteristics == "Employment",
    sex == "Both sexes",
    age_group == "15 years and over",
...
ggplotly_lfs(change_employment_provinces)
```
