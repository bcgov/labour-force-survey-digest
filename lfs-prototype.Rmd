...
### Month-Over-Month Percent Change in Number of Jobs for `r paste(report_month, report_year)` by B.C. Economic Region 
...
### B.C. Unemployment Rate for Woman Aged 25 Years & Over {.value-box}

```{r}
#month bc unemployment rate - females
valueBox(
  ...
  filter(
    ...
    signs(format = percent, accuracy = 0.1),
  # icon = "fa-users-cog",
  caption = paste("B.C. Unemployment Rate for Woman Aged 25 Years & Over for", report_month, report_year)
)
```


### B.C. Unemployment Rate for Men Aged 25 Years & Over {.value-box}

```{r}
#month bc unemployment rate - males
valueBox(
  value = lfc_province_tidy %>%
  filter(
    vector == "v2064822",
    ...
    signs(format = percent, accuracy = 0.1),
  # icon = "fa-users-cog",
  caption = paste("B.C. Unemployment Rate for Men Aged 25 Years & Over for", report_month, report_year)
)
```
...
### Monthly B.C. Unemployment Rates by Gender & Age Since 2000
...
### Number of Jobs for Woman Aged 25 Years & Over in B.C. {.value-box}

```{r}
#recent monthly change in employment females
valueBox(
  value = lfc_province_tidy %>%
  filter(
    vector == "v2064827",
    ...
    signs(format = comma),
  # icon = "fa-users-cog",
  caption = paste("Number of Jobs for Woman in B.C. for", report_month, report_year, "(Aged 25 Years & Over)")
)
```

### Year-Over-Year Change in Number of Jobs for Woman Aged 25 Years & Over in B.C. {.value-box}

```{r}
year_change_employment_bc_woman <- lfc_province_tidy %>%
  filter(
    vector == "v2064827",
    date %in% c(max(date) - months(12), max(date))
  ) %>%
...
# year over year change employment
valueBox(
  value = year_change_employment_bc_woman %>%
    pull(year_change) %>%
    signs(
      format = comma,
      add_plusses = TRUE
  ...
  # icon = "fa-users-cog",
  caption = paste("Year-Over-Year Change in Number of Jobs for Woman in B.C.")
)
```


### Number of Jobs for Men Aged 25 Years & Over in B.C. {.value-box}}

```{r}
#recent monthly change in employment males
valueBox(
  value = lfc_province_tidy %>%
  filter(
    vector == "v2064818",
    ...
    signs(format = comma),
  # icon = "fa-users-cog",
  caption = paste("Number of Jobs for Men in B.C. for", report_month, report_year, "(Aged 25 Years & Over)")
)
```

### Year-Over-Year Change in Number of Jobs for Men Aged 25 Years & Over in B.C. {.value-box}

```{r}
year_change_employment_bc_men <- lfc_province_tidy %>%
  filter(
    vector == "v2064818",
    date %in% c(max(date) - months(12), max(date))
  ) %>%
...
# year over year change employment
valueBox(
  value = year_change_employment_bc_men %>%
    pull(year_change) %>%
    signs(
      format = comma,
      add_plusses = TRUE
...
### Month-Over-Month Percent Change in Number of Jobs in B.C. by Gender for in `r report_year` (Ages 25 Years & Over)
...
ggplotly_lfs(
  ...
  geom_text(
    ...
    nudge_y = ifelse(.$month_change_percent > 0, .003, -.003),
    colour = "grey20",
    size = 2
  ) +
  labs(x = NULL,
...
### Month-Over-Month Percent Change in Number of Jobs in B.C. by Age in `r report_year` 
...
ggplotly_lfs(
  ...
  geom_text(
    ...
    nudge_y = ifelse(.$month_change_percent > 0, .01, -.01),
    colour = "grey20",
    size = 2
  ) +
  labs(x = NULL,
