...
🖱 Click or tap on graphs for some interactive options:

![](images/plotly-options.png){width=90%}

<br>

Learn more [About] the Statistics Canada Labour Force Survey data.
...
valueBox(
  value = recent_month_unemployment_rate_bc %>%
    ...
    signs(format = percent,
          accuracy = 0.1,
          suffix = ""),
  caption = "Month-Over-Month % Point Change in B.C. Unemployment Rate"
)
```
...
valueBox(
  ...
  # icon = "fa-users-cog",
  caption = paste("Year-Over-Year % Point Change in B.C. Unemployment Rate")
)
```
...
unemployrt <- lfc_province_tidy %>%
  filter(vector == "v2064705",
         date > "1999-12-31") %>% 
         mutate(timeseries = case_when(date >= paste0(report_year, "-01-01") ~ "current_year",
                              TRUE ~ "previous_years"))

ggplotly_lfs(
  ggplot(data = unemployrt,
    aes(
      x = date,
      y = value, 
      colour = timeseries)) +
  ...
  scale_colour_manual(guide = FALSE,
                      values = c(previous_years = "grey50",
                                 current_year = main_colour)) +
  theme_minimal() +
  theme_vert +
...
lfc_region_tidy %>%
    ...
    # coord_sf(datum = NA) +
    labs(x = NULL, y = NULL,
         caption = "(3-month-moving-avg, seasonally unadjusted estimates)") +
    scale_fill_viridis(
...
valueBox(
  value = month_unemployment_rate_can %>%
    ...
    signs(format = percent,
          ...
          suffix = ""),
  # icon = "fa-users-cog",
  caption = paste("Month-Over-Month % Point Change in Canada Unemployment Rate")
)
```
...
valueBox(
  ...
  # icon = "fa-users-cog",
  caption = paste("Year-Over-Year % Point Change in Canada Unemployment Rate")
)
```
...
gt_lfs_uer <- function(tbl) {
  ...
  mutate(name_label = case_when(
    name == "value" ~ toTitleCase(labour_force_characteristics),
    name == "month_change" ~ "Month-Over-Month % Point Change",
    name == "year_change" ~ "Year-Over-Year % Point Change",
    TRUE ~ "there is a problem"
  ), .after = name) %>% 
  gt() %>%
  tab_style(
...
year_change_unemployment_bc_female %>%
  ...
  fmt_percent(rows = name == "value",
    ...
    case_when(x == 0 ~ percent(x, accuracy = .1),
              x > 0 ~ paste(percent(x, accuracy = .1, suffix = ""), up_arrow),
              x < 0 ~ paste(percent(x, accuracy = .1, suffix = ""), down_arrow))
    }
  )
```
...
### Monthly B.C. Unemployment Rates for Women & Men Since 2000 (Ages 25 Years & Over)
...
lfc_province_tidy %>%
  ...
  mutate(highlight = case_when(date >= paste0(report_year, "-01-01") ~ "current_year",
                               TRUE ~ "previous_years"),
         hover_labels = case_when(sex == "Males" ~ "Men",
                                  sex == "Females" ~ "Women",
                                  TRUE ~ sex)) %>% 
  ggplot(aes(
    x = date,
  ...
  facet_wrap(facets = vars(sex),
             labeller = as_labeller(facet_sex_labels)) +
  labs(x = NULL,
       y = NULL) +
  scale_y_continuous(labels = percent,
                     breaks = breaks_pretty(n = 6)) +
...
### Month-Over-Month Change in Number of Jobs for Women & Men in B.C. for `r report_year` (Ages 25 Years & Over)
...
### Monthly B.C. Unemployment Rates by Age for Women & Men for `r report_year`
...
lfc_province_tidy %>%
  ...
  mutate(hover_labels = case_when(sex == "Males" ~ "Men",
                                  sex == "Females" ~ "Women",
                                  TRUE ~ sex)
         ) %>% 
  ggplot(aes(
...
### Month-Over-Month Change in Number of Jobs for Women & Men Youth in B.C. for `r report_year` (Ages 15 to 24 Years)

```{r, fig.width=7, fig.height=5}

##labels
youth_sex_labels <-  c("Males" = "Youth: Men",
                          "Females" = "Youth: Women")
ggplotly_lfs(
  lfc_province_tidy %>%
  filter(
  ...
  geom_col(fill = main_colour,
           alpha = 0.6) +
  facet_wrap(facets = vars(sex),
             labeller = as_labeller(youth_sex_labels)) +
  geom_text(
    aes(
      label = signs(
        month_change,
...
###  Changes in Number of Jobs in B.C. by Industry for `r paste(report_month, report_year)` {.no-mobile}
...
valueBox(
  ...
  # icon = "fa-users-cog",
  caption = paste("% Point Change in B.C. Unemployment Rate Since February 2020")
)
```
...
### Percentage Point Change in B.C. Unemployment Rate by Age for Women & Men Since February 2020

```{r}
feb_change_unemployment_bc_sex <- lfc_province_tidy %>%
  filter(
   date %in% c(max(date), "2020-02-01"),
    geo == "British Columbia",
    labour_force_characteristics %in% c("Unemployment rate"),
...
cols_mf <- c("Males", "Females")
  ...
  gt(feb_change_unemployment_bc_sex) %>%
  fmt_percent("Males", decimals = 1) %>% 
    ...
    fns = function(x) {
    case_when(x == 0 ~ percent(x, accuracy = .1, suffix=""),
              x > 0 ~ paste(percent(x, accuracy = .1, suffix=""), up_arrow), 
              x < 0 ~ paste(percent(x, accuracy = .1, suffix=""), down_arrow))
    }
  )
...
### Change in Number of Jobs in B.C. by Job Type for Women & Men Since February 2020

```{r}
feb_change_employment_bc_sex <- lfc_province_tidy %>%
  filter(
    date %in% c(max(date), "2020-02-01"),
    geo == "British Columbia",
    labour_force_characteristics %in% c("Full-time employment",
...
gt(feb_change_employment_bc_sex) %>%
    cols_label(
    Females = "Women",
    Males = "Men",
    labour_force_characteristics = ""
...
Row 3 {data-height=1210}
------------------------------------------------------------------------

### Change in Number of Industry Jobs in B.C. by Age for Women & Men Since February 2020 {.no-mobile}
...
industry_employ_long <- lfs_industry_unadjusted_tidy %>%
  ...
  unite(sex_age, c(sex, age_group)) 

industry_employ <- industry_employ_long %>% 
  pivot_wider(names_from = sex_age,
              values_from = feb_change)

max_scale <- pmax(abs(min(industry_employ_long$feb_change, na.rm = TRUE)),
                  max(industry_employ_long$feb_change, na.rm = TRUE))
...
gt(industry_employ,
   ...
   tab_header(
    title = "Change in Number of Jobs Since February 2020", 
    subtitle = "Based on monthly, seasonally unadjusted estimates"
  ) %>% 
   tab_spanner(
   ...
   tab_source_note(
    source_note = "Note: seasonal or calendar effects that influence employment are not accounted for when using monthly, seasonally unadjusted employment estimates."
  ) %>% 
    tab_source_note(
...
Row 4 {data-height=1250}
-------------------------------------------------------------------------

### Percentage Point Change in B.C. Industry Unemployment Rates by Age for Women & Men Since February 2020 {.no-mobile}
...
max_scale_perc <- pmax(abs(min(industry_unemployrt_long$feb_change, na.rm = TRUE)),
                  max(industry_unemployrt_long$feb_change, na.rm = TRUE))
                                                                     
scale_range_perc <- c(-max_scale_perc, max_scale_perc)

gt(industry_unemployrt,
     rowname_col = "industry") %>% 
  tab_header(
    title = "Percentage Point Change in Unemployment Rate Since February 2020", 
    subtitle = "Based on monthly, seasonally unadjusted estimates"
  ) %>% 
   tab_spanner(
    label = "15 to 24 Years",
  ...
  tab_source_note(
    source_note = "Note: seasonal or calendar effects that influence unemployment rates are not accounted for when using monthly, seasonally unadjusted unemployment rate estimates."
  ) %>% 
    tab_source_note(
    source_note = md("[Statistics Canada Table **14-10-0022-01**: Labour force characteristics by industry, monthly, unadjusted for seasonality](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=1410002201)")
  ) %>% 
   ...
   fmt(
    ...
    fns = function(x) {
      case_when(is.na(x) ~ "x",
    TRUE ~ paste(percent(x, accuracy = .1, suffix = "")))
    }
  )
# %>% 
#    fmt(
...
Labour force characteristic time series data are based on [monhtly, seasonally adjusted values](https://www.statcan.gc.ca/eng/dai/btd/sad-faq/sec1) unless otherwise indicated in the dashboard.
