...
lfs_industry_unadjusted_tidy %>%
  ...
  mutate(
    ...
    feb_change_percent = value / lag(value) - 1
  ) %>%
  filter(date == max(date)) %>% 
  mutate(industry = str_extract(naics, "^[^\\[]+")) %>%
  ungroup() %>% 
  select(industry, sex, age_group, vector, value, month_change,
         month_change_percent, feb_change, feb_change_percent) 
